...
const [endDate, setEndDate] = useState(null);
  ...
  const buscarReportes = () => {
    const filt = dreportes.filter((reporte) => {
      ...
      (!startDate || reportDate >= startOfDay(new Date(startDate))) &&
      (!endDate || reportDate <= endOfDay(new Date(endDate)))
    );
  }
  ...
  selected={startDate}
  onChange={(date) => {
    setStartDate(date); // Actualiza el estado con el objeto Date
    buscarReportes(); // Aplica todos los filtros automáticamente
  }}
  selectsStart
  startDate={startDate}
  endDate={endDate}
...
<DatePicker.default
  selected={endDate}
  onChange={(date) => {
    setEndDate(date); // Actualiza el estado con el objeto Date
    buscarReportes(); // Aplica todos los filtros automáticamente
  }}
  selectsEnd
  startDate={startDate}
