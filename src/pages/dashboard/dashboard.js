...
const [endDate, setEndDate] = useState(null);
  ...
  const buscarReportes = () => {
    const filt = dreportes.filter((reporte) =>{
      const reportDate = new Date(reporte.fchareg);
      return(
      ...
      (!startDate || reportDate >= startOfDay(startDate)) &&
      (!endDate || reportDate <= endOfDay(endDate))
    );
  });
    setReportes(filt);
  };
  ...
  onChange={(date) => {
    setStartDate(date);
    buscarReportes(); // Apply all filters automatically
  }}
  selectsStart
  startDate={startDate}
  endDate={endDate}
